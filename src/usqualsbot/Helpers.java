...
public class Helpers {
            ...
            while (queueStart < levelEnd) {
                ...
                for (int dx = -1; dx <= 1; dx++) {
                    for (int dy = -1; dy <= 1; dy++) {
                        ...
                        // Create MapLocation for remaining checks
                        MapLocation neighbor = new MapLocation(nx, ny);
                        
                        // Map bounds check
                        if (!rc.onTheMap(neighbor)) continue;
                        
                        // Passability check
                        Tile tile = get(neighbor);
    ...
    // Legacy method kept for compatibility - calls the new optimized version
    static boolean runBFS(MapLocation start, FastSetPrecise enemyLocs) {
        // Convert FastSetPrecise to RobotInfo array format
        // This is for backward compatibility - prefer using canReachEnemyInMoves directly
        MapLocation[] locs = enemyLocs.getLocations();
        
        // Clear and use enemy bitset directly
        for (int i = 0; i < 64; i++) {
            visitedBits[i] = 0L;
            enemyBits[i] = 0L;
        }
        ...
        minEX = Math.max(0, minEX - 3);
        maxEX = Math.min(mapWidth - 1, maxEX + 3);
        minEY = Math.max(0, minEY - 3);
        maxEY = Math.min(mapHeight - 1, maxEY + 3);
            ...
            while (queueStart < levelEnd) {
                ...
                for (int ddx = -1; ddx <= 1; ddx++) {
                    for (int ddy = -1; ddy <= 1; ddy++) {
                        ...
                        if ((enemyBits[ny] & (1L << nx)) != 0) return true;
    ...
    static boolean isRatnappable(RobotInfo target, MapLocation fromLoc) {
        ...
        if (rc.getHealth() > target.getHealth())
            return true;
