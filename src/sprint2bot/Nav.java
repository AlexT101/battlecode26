...
public class Nav {
    ...
    static boolean tryGreedyMove() throws GameActionException {
        ...
        if (canPass(dir1)) dist1 = myLoc.add(dir1).distanceSquaredTo(prevTarget);
        
        Direction dir2 = dir.rotateLeft();
        if (canPass(dir2)) dist2 = myLoc.add(dir2).distanceSquaredTo(prevTarget);
    ...
    static void checkRotate(Direction dir) throws GameActionException {
        ...
        int i = 8;
        while (--i >= 0) {
            if (!canPass(dirLeft)) dirLeft = dirLeft.rotateLeft();
            else break;
        }
        i = 8;
        while (--i >= 0) {
            if (!canPass(dirRight)) dirRight = dirRight.rotateRight();
            else break;
        }
    ...
    // Returns true if we can move OR if it is dirt and we have enough cheese to dig it.
    static boolean canPass(Direction dir) throws GameActionException {
        ...
        if (rc.onTheMap(next) && rc.canSenseLocation(next)) {
            ...
            if (info.isDirt()) {
                return rc.canRemoveDirt(next) && rc.getGlobalCheese() > CHEESE_EMERGENCY_THRESHOLD;
            }
        }
        return false;
    }
