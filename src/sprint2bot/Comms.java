...
public class Comms {
     ...
     */
    static int encodeFullLocation(MapLocation loc) {
    ...
    static void writeMineLocation(MapLocation mineLoc) throws GameActionException {
        ...
        int encodedMine = Comms.encodeLocation(mineLoc);
        ...
        if (symmetry != Symmetry.UNKNOWN) {
            ...
            symmetricalEncoded = Comms.encodeLocation(symLoc);
        }
    ...
    static void writeCondensedMines() throws GameActionException {
        ...
        for (int i = 0; i < currentMineCount; i++) {
            ...
            MapLocation symLoc = getSymmetricLocation(mineLoc, symmetry);
            int symmetricalEncoded = Comms.encodeLocation(symLoc);
    ...
    static void squeakMineLocations() throws GameActionException {
        ...
        for (int encodedMine : mineLocations.getValues()) {
            ...
            for (int i = 0; i < mineCount; i++) {
                ...
                // If symmetry is known, check if the symmetrical location is stored
                if (symmetry != Symmetry.UNKNOWN && storedMine == symmetricalEncoded) {
                    inSharedArray = true;
                    break;
                }
            }
    ...
    static void updateCachedKingLocations() throws GameActionException {
        for (int i = 0; i < 5; i++) {
