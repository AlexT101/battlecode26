...
public class RobotPlayer {
    ...
    static final int CHEESE_SURPLUS_THRESHOLD = 1500;
    static final int MAX_ALIVE_RATS_CHEESE_SURPLUS = 22;
    static final int LOTS_OF_CHEESE_THRESHOLD = 2200;
    static final int MAX_ALIVE_RATS_LOTS_OF_CHEESE = 32;
    static final int MIN_ALIVE_RATS = 5;
    ...
    static Direction findBestThrowDirection() throws GameActionException {
        ...
        for (Direction dir : DIRS) {
            ...
            try {
                if (!rc.canSenseLocation(throwLoc))
                    continue;
        ...
        for (Direction dir : DIRS) {
            ...
            if (!rc.onTheMap(throwLoc))
                continue;
            try {
                if (rc.canSenseRobotAtLocation(throwLoc) && rc.sensePassability(throwLoc)
                        && rc.senseRobotAtLocation(throwLoc) == null) {
                    return dir;
                }
            } catch (GameActionException e) {
                continue;
    ...
    static void handleTrapPlacement() throws GameActionException {
        ...
        if (enemyKing != null) {
            ...
            if (withinTwoTiles && cheese >= CHEESE_CRITICAL_THRESHOLD) {
                Direction toKing = myLoc.directionTo(kingCenter);

                tryPlaceTrapInDirection(toKing);
                ...
                if (oneTileAway) {
                    ...
                    tryPlaceTrapInDirection(left);
                    tryPlaceTrapInDirection(right);
                }
            }
        }
        ...
        if (enemyRatCount >= 1 && isNearAllyRatKing() && rc.getAllCheese() >= CHEESE_CRITICAL_THRESHOLD) {
            Direction towardEnemies = getDirectionTowardEnemyRats();
            if (towardEnemies != null) {
                tryPlaceTrapInDirection(towardEnemies);
            }
        }

        // If 3+ enemy rats visible and none adjacent, place trap toward them
        if (enemyRatCount >= 3 && !hasAdjacent && rc.getAllCheese() >= CHEESE_EMERGENCY_THRESHOLD) {
            Direction towardEnemies = getDirectionTowardEnemyRats();
            if (towardEnemies != null) {
                tryPlaceTrapInDirection(towardEnemies);
            }
    ...
    static void runBabyRat() throws GameActionException {
        ...
        if (rc.isBeingThrown()) {
            ...
            MapLocation oneAway = myLoc.add(throwDirection);
            if (rc.canSenseLocation(oneAway)) {
                RobotInfo first = rc.senseRobotAtLocation(oneAway);
            ...
            MapLocation twoAway = myLoc.add(throwDirection).add(throwDirection);

            if (rc.canSenseLocation(twoAway)) {
