...
public class FastSetPrecise implements Iterable<Character> {
    private StringBuilder values;

    public FastSetPrecise() {
    ...
    public boolean add(MapLocation location) {
        return add(encodeFullLocation(location));
    }

    public boolean remove(MapLocation location) {
        char encodedLocation = encodeFullLocation(location);
    ...
    public boolean contains(MapLocation location) {
        return contains(encodeFullLocation(location));
    }
    ...
    public MapLocation[] getLocations() {
        ...
        for (int i = 0; i < chars.length; i++) {
            locations[i] = decodeFullLocation(chars[i]);
        }
        return locations;
    }
